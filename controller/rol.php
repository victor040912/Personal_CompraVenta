    ...
    switch($_GET["op"]){
        ...
        case "listar":
            ...
            foreach($datos as $row){
                ...
                $sub_array[] = '<button type="button" onClick="permiso('.$row["ROL_ID"].')" id="'.$row["ROL_ID"].'" class="btn btn-primary waves-effect waves-light"><i class="ri-file-settings-line"></i></button>';
                $sub_array[] = '<button type="button" onClick="editar('.$row["ROL_ID"].')" id="'.$row["ROL_ID"].'" class="btn btn-warning waves-effect waves-light"><i class="ri-edit-2-line"></i></button>';
